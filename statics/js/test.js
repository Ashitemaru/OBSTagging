...
function processProblemContent(content) {
    ...
    let screen_name = content[1];
    let time_all = content[2];
    let twitter = content[3];
    g_true_answer = [];
    for (let i = 4; i < 12; i++) {
        g_true_answer.push(parseInt(content[i]));
    }
    g_tips = content[12];
    console.log(g_true_answer);
    console.log(g_tips);
    twitter = twitter.replace(/\\n/g, "<br>")
    let html_str = '<h4> Screen name:'+screen_name+'</h4><h4> Time:'+time_all+
            '</h4><div id=\'twitter_content\'> Content:<br>'+twitter+'</div><div id=\'tips\'></div>'
    return html_str
}

function problem_choice_html_by_id(id, item_name){
    choice = '<div>\
                ...
                <div class="input-container">\
                  <input id="unrealted" class="radio-button" type="radio" name="{{radio_name}}" value="0" /> \
                  <div class="radio-tile">\
                    <div class="icon unrealted-icon">\
                      \
                    </div>\
                    <label for="unrealted" class="radio-tile-label">无关</label>\
                  </div>\
                </div>\
\
                <div class="input-container">\
                  <input id="negative" class="radio-button" type="radio" name="{{radio_name}}" value="1" />\
                  <div class="radio-tile">\
                    <div class="icon negative-icon">\
                      \
                    </div>\
                    <label for="negative" class="radio-tile-label">负面</label>\
                  </div>\
                </div>\
\
                <div class="input-container">\
                  <input id="neutral" class="radio-button" type="radio" name="{{radio_name}}" value="2" />\
                  <div class="radio-tile">\
                    <div class="icon car-icon">\
                      \
                    </div>\
                    <label for="neutral" class="radio-tile-label">中性</label>\
                  </div>\
                </div>\
\
                <div class="input-container">\
                  <input id="positive" class="radio-button" type="radio" name="{{radio_name}}" value="3" />\
                  <div class="radio-tile">\
                    <div class="icon positive-icon">\
                      \
                    </div>\
                    <label for="positive" class="radio-tile-label">正面</label>\
                  </div>\
                </div>\
              </div>\
              </div>'

    choice = choice.replace(/{{radio_name}}/g, id);
    choice = choice.replace(/{{item_name}}/g, item_name);
...
function showQuestion(id) {
    ...
    for (let i = 0; i < col.length; i++) {
        ...
        padding = 6-item_name.length;
        for (let j = 0; j < padding; j++) {
            item_name += '&emsp;';
        }
        question_display += problem_choice_html_by_id("radio_" + i,item_name);
    }
...
function writeAnswer() {
    ...
    let tips_content = '';
    for (let i = 0; i < 8; i++) {
        if(g_true_answer[i] != checkQues[g_activeQuestion_id].answer[i]){
            wrong_answer = true;
            tips_content += col[i] + '应选择' + col_dict[g_true_answer[i]] + '<br>';
        }
    }
    if(wrong_answer){
        tips_content += g_tips;
        $('#tips').html(tips_content);
        return false;
    }
    return true;
}
...
$(function() {
    ...
    for (let i = 0; i < problem_list.length; i++) {
        ...
        answerTMP.answer = [0,0,0,0,0,0,0,-1];
